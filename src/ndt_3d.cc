...
// #include <execution>  // 包含std::execution::par_unseq
#include <ros/ros.h>
...
namespace sad
{
    ...
    void Ndt3d::BuildVoxels()
    {
        ...
        if (target_ == nullptr)
        {
            ROS_WARN_STREAM_THROTTLE(1, "NDT_3D:NO MAP!");
            return;
        }

        // assert(target_->empty() == false);
        if (target_->empty() == true)
        {
            ROS_WARN_STREAM_THROTTLE(1, "NDT_3D:MAP EMPTY!");
            return;
        }
        ...
        if (v.second.idx_.size() > options_.min_pts_in_voxel_) {
            ...
            Eigen::JacobiSVD<Eigen::Matrix<double, 3, 3>> svd(v.second.sigma_, Eigen::ComputeFullU | Eigen::ComputeFullV);
    ...
    bool Ndt3d::AlignNdt(SE3 &init_pose)
    {
        ...
        // ROS_INFO("aligning with ndt");

        // assert(grids_.empty() == false);
        if (grids_.empty() == true)
        {
            // ROS_WARN_STREAM_THROTTLE(1, "NDT_3D:GRIDS EMPTY!");
            return false;
        }
        ...
        if (options_.remove_centroid_)
        {
            ...
            ROS_INFO("init trans set to %f %f %f", pose.translation()(0), pose.translation()(1), pose.translation()(2));
        }
        ...
        for (int iter = 0; iter < options_.max_iteration_; ++iter)
        {
            ...
            for (int idx = 0; idx < effect_pts.size(); ++idx)
            {
                ...
                total_res += res;      //所有邻近栅格的残差-ff
                ndt_score += std::exp(-res/2.0);                 // 匹配得分累加-0706
            ...
            if (effective_num < options_.min_effective_pts_)
            {
                ...
                // ROS_WARN("effective num too small: %d", effective_num);
            ...
            ndt_matching_score_ = ndt_score / index.size(); //0706

            // ROS_INFO("本次匹配得分: %f", ndt_matching_score_);
            ...
            //           << ", dx: " << dx.transpose();
            ndt_iter_ = iter;
            // ROS_INFO("iter %d total res: %f, eff: %d, mean res: %f, dxn: %f, dx: %f %f %f %f %f %f",
            //          iter, total_res, effective_num, total_res / effective_num, dx.norm(),
            //          dx[0], dx[1], dx[2], dx[3], dx[4], dx[5]);
            // ROS_INFO("dx.norm(): %f", dx.norm());
            ...
            if (gt_set_)
            {
                ...
                // ROS_INFO("iter %d pose error: %f", iter, pose_error);
            }
            ndt_dx_ = dx.norm();
            if (dx.norm() < options_.eps_)
            {
                // LOG(INFO) << "converged, dx = " << dx.transpose();
                // ROS_INFO("converged, dx = %f %f %f %f %f %f", dx(0), dx(1), dx(2), dx(3), dx(4), dx(5));
                ndt_ailgn_ = true;
                break;
            }
        }
